...
; (async () => {
  ...
  if (result.length) await fs.writeFileSync(base + 'gempa_terkini.json', JSON.stringify(result, null, 2))
})()
...
; (async () => {
  ...
  if (result.length) await fs.writeFileSync(base + 'gempa_dirasakan.json', JSON.stringify(result, null, 2))
})()
...
; (async () => {
  ...
  for (let i = 0; i < (tr.length || 50); i++) {
    ...
    if (!data.html) continue
    let waktu = $(data).eq(0).html().replace('<br>', ' ').trim()
  ...
  if (result.length) await fs.writeFileSync(base + 'tsunami.json', JSON.stringify(result, null, 2))
})()