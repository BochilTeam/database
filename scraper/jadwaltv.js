...
; (async () => {
  ...
  if (Object.keys(result).length) await fs.writeFileSync(base + 'jadwaltv.json', JSON.stringify(result, null, 2))
})()
...
; (async () => {
  ...
  if (Object.keys(result).length) await fs.writeFileSync(base + 'acara_aktual.json', JSON.stringify(result, null, 2))
})()
